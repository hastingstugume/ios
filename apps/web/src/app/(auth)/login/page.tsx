...
import { getPostAuthRedirect } from '@/lib/auth-flow';
...
function LoginPageContent() {
  ...
  const [mfaChallengeToken, setMfaChallengeToken] = useState(searchParams.get('challenge') || '');
  ...
  useEffect(() => {
    const nextChallenge = searchParams.get('challenge') || '';
    if (nextChallenge) setMfaChallengeToken(nextChallenge);
  }, [searchParams]);
  ...
  const login = useMutation({
    ...
    onSuccess: (result) => {
      ...
      const destination = getPostAuthRedirect(result.authState);
  ...
  const verifyMfa = useMutation({
    mutationFn: () => authApi.verifyMfaLogin({ challengeToken: mfaChallengeToken, code: mfaCode }),
    onSuccess: (result) => {
      qc.invalidateQueries({ queryKey: ['auth', 'me'] });
      const destination = getPostAuthRedirect(result.authState);
      if (destination === '/verify-email') {
  ...
  const mfaError = verifyMfa.error ? (verifyMfa.error as Error).message : null;
      ...
      <div className="space-y-4">
        {!mfaChallengeToken ? (
          ...
          <div className="space-y-4 rounded-2xl border border-border bg-secondary/35 p-4">
            <div>
              <p className="text-sm font-medium text-foreground">Enter your authentication code</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Use the 6-digit code from your authenticator app, or enter one of your backup codes.
              </p>
            </div>
            <div>
              <label className="text-sm text-muted-foreground mb-1.5 block">Authentication code</label>
              <input
                value={mfaCode}
            ...
            {mfaError ? (
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2">
                <p className="text-destructive text-sm">{mfaError}</p>
              </div>
            ) : null}
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              >
                {verifyMfa.isPending ? 'Verifying…' : 'Continue'}
              </button>
              <button
                type="button"
                onClick={() => {
        ...
        {!mfaChallengeToken && needsVerification && email ? (
