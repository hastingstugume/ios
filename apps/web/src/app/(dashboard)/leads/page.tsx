...
import { LeadCard } from '@/components/signals/LeadCard';
...
function StatChip({ label, value, icon: Icon }: { label: string; value: number; icon: any }) {
  return (
    <div className="min-w-0 flex-1 rounded-lg border border-border bg-background px-3 py-2 sm:min-w-[120px]">
...
export default function FeedPage() {
  ...
  const [leadFinder, setLeadFinder] = useState({
    niche: 'Home services',
    market: 'United States',
    offer: 'Local SEO and GBP optimization',
  });
  ...
  const newLeadCount = data?.data?.filter((signal) => signal.status === 'NEW').length ?? 0;
  const applyLeadFinder = () => {
    const parts = [leadFinder.niche, leadFinder.offer, leadFinder.market]
      ...
      <div className="page-hero space-y-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Review qualified prospects, prioritize who needs help now, and move the best accounts into pipeline.</p>
          </div>
          <button
          ...
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
            <div className="min-w-0 xl:w-52">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Start from niche, market, and offer.
              </p>
            </div>
            <div className="grid flex-1 gap-2 md:grid-cols-3">
              <label className="space-y-1">
                ...
                <input
                  value={leadFinder.niche}
                  onChange={(e) => setLeadFinder((current) => ({ ...current, niche: e.target.value }))}
              ...
              <label className="space-y-1">
                ...
                <input
                  value={leadFinder.market}
                  onChange={(e) => setLeadFinder((current) => ({ ...current, market: e.target.value }))}
              ...
              <label className="space-y-1">
                ...
                <input
                  value={leadFinder.offer}
                  onChange={(e) => setLeadFinder((current) => ({ ...current, offer: e.target.value }))}
                  placeholder="Local SEO and GBP optimization"
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/40"
                />
              </label>
            </div>
            <div className="flex items-center gap-3 xl:pl-2">
              <Link
                href="/sources"
        ...
        <div className="grid grid-cols-2 gap-2 lg:flex lg:flex-wrap">
          <StatChip label="New Leads" value={newLeadCount} icon={Clock} />
          <StatChip label="High Fit" value={highConfidenceCount} icon={Target} />
        ...
        <div className="flex items-center justify-end">
          <span className="text-xs text-muted-foreground">
            {trustedSourceCount} on this page came from production-ready sources
          </span>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="section-card px-6 py-10">
          <div className="mx-auto max-w-3xl space-y-4">
            <div className="text-center">
              <Target className="mx-auto mb-3 h-9 w-9 text-muted-foreground/30" />
              <h3 className="text-base font-medium text-foreground">No leads yet</h3>
              <p className="mx-auto mt-1 max-w-xl text-sm text-muted-foreground">
                Let&apos;s unblock your first prospects. Use these quick checks:
              </p>
            </div>
            ...
            <div className="space-y-2.5">
              {activeFilterCount > 0 ? (
                <div className="flex flex-col gap-3 rounded-lg border border-border bg-secondary/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-foreground">Your current filters may be hiding matching leads.</p>
                  </div>
                  <button
                    onClick={() =>
              ...
              <div className="flex flex-col gap-3 rounded-lg border border-border bg-secondary/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-foreground">
                  {hasNoActiveSources
                    ? 'Connect at least one active source to start collecting prospects.'
              ...
              <div className="flex flex-col gap-3 rounded-lg border border-border bg-secondary/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-foreground">
                  {hasNoActiveKeywords
                    ? 'Add a few intent-heavy rules so the best prospects rank higher.'
                    : `${activeKeywords.length} active keyword${activeKeywords.length === 1 ? '' : 's'} shaping lead quality.`}
                </p>
                <Link href="/keywords" className="rounded-md border border-border px-3 py-1.5 text-sm text-foreground transition-colors hover:bg-accent">
                  {hasNoActiveKeywords ? 'Add intent rules' : 'Refine intent rules'}
                </Link>
              </div>
              ...
              {noSignalsYet && primarySource ? (
                <div className="flex flex-col gap-3 rounded-lg border border-border bg-secondary/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-foreground">
                    Run a fresh pull on <span className="font-medium">{primarySource.name}</span> to surface your first leads faster.
                  </p>
                  <button
                    onClick={() => {
              ...
              {likelyCredentialBlockedSources.length > 0 ? (
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-800 dark:text-amber-200">
                  Some sources have never fetched. If you are using Reddit or web-search connectors, verify provider credentials in your backend environment.
                </div>
              ) : null}
            </div>
          </div>
        ...
        <div className="grid gap-3">
          {data.data.map((signal) => (
            <LeadCard key={signal.id} signal={signal} orgId={currentOrgId!} queryKey={queryKey} />
          ))}
        </div>
      )}
